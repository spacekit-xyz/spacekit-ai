...
pub fn forward(x: &[f32], gamma: &[f32], beta: &[f32], eps: f32) -> (Vec<f32>, StandardNormStats) {
    ...
    let y: Vec<f32> = x_hat
        ...
        .map(|((&xh, &g), &b)| xh * g + b)
        .collect();
    (y, StandardNormStats { x_hat, mean, std })
}

/// Backward for `y = x_hat * gamma + beta`.
pub fn backward(x_hat: &[f32], gamma: &[f32], grad_out: &[f32], std: f32) -> Vec<f32> {
    let n = x_hat.len();
    ...
    (0..n)
        .map(|i| inv_std * (grad_x_hat[i] - inv_n * sum1 - inv_n * x_hat[i] * sum2))
        .collect()
}
