...
fn geometric_product_blades(a: usize, b: usize, metric: &[f32]) -> (f32, usize) {
...
impl CliffordLinear {
    pub fn new(in_dim: usize, out_dim: usize, algebra: Arc<CliffordAlgebra>) -> Self {
        ...
        let weights = (0..out_dim)
            .map(|_| (0..in_dim).map(|_| Multivector::scalar(0.01)).collect())
            .collect();
...
impl CliffordAttention {
    ...
    /// x: [seq_len][d_model] multivectors
    pub fn forward(&self, x: &[Vec<Multivector>]) -> Vec<Vec<Multivector>> {
        ...
        for i in 0..seq {
            // Attention score: sum of grade-0 parts across d_model geometric products
            // score(i,j) = (1/√d) Σ_d <Q_i[d] ⊛ K_j[d]>₀
            let raw_scores: Vec<f32> = (0..seq)
                .map(|j| {
                    let s: f32 = q[i]
                        .iter()
                        .zip(k[j].iter())
                        .map(|(qi, kj)| self.algebra.geo_product(qi, kj).scalar_part())
                        .sum();
                    s / scale
                })
                .collect();
...
pub struct CliffordLLM {
    ...
    pub head: CliffordLinear, // d_model → vocab_size (project grade-0 to logits)
    pub algebra: Arc<CliffordAlgebra>,
}
...
impl CliffordLLM {
    ...
    pub fn forward(&self, token_ids: &[usize]) -> Vec<Vec<f32>> {
        ...
        // 3. Project to logits:
        //    For each position, project d_model multivectors → vocab_size scalars
        //    by extracting the grade-0 (scalar) part from the output head
        x.iter()
            .map(|xi| {
                self.head
                    .forward(xi)
                    .iter()
                    .map(|mv| mv.scalar_part())
                    .collect()
            })
            .collect()
    }
...
mod tests {
    ...
    fn forward_pass_shape() {
        ...
        let head = CliffordLinear::new(d_model, vocab, alg.clone());
