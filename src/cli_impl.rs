...
fn apply_gf_project(args: &mut Args, project_path: &Path) -> Result<GfOverlay, String> {
    ...
    let base = crate::project_gf::manifest_base_dir(project_path);
    ...
    if let Some(tr) = &gf.train {
        ...
        if args.data_dir.is_none() {
            if let Some(d) = &tr.data_dir {
                args.data_dir = Some(
                    crate::project_gf::resolve_against(&base, d)
                        .to_string_lossy()
                        .into_owned(),
                );
            }
        ...
        if args.brain_plugins_toml.is_none() {
            if let Some(p) = &tr.brain_plugins_toml {
                args.brain_plugins_toml = Some(crate::project_gf::resolve_against(&base, p));
            }
        }
        if args.brain_output.is_none() {
            if let Some(o) = &tr.brain_output {
                args.brain_output = Some(
                    crate::project_gf::resolve_against(&base, o)
                        .to_string_lossy()
                        .into_owned(),
                );
            }
        ...
        if let Some(g) = &tr.gle_checkpoint {
            let resolved = crate::project_gf::resolve_against(&base, g);
    ...
    if let Some(inf) = &gf.inference {
        if args.inference_toml.is_none() {
            if let Some(t) = inf.toml.as_deref() {
                args.inference_toml = Some(crate::project_gf::resolve_against(&base, t));
            }
        }
    ...
    if let Some(inf) = &gf.infer {
        if args.brain.is_none() {
            if let Some(b) = &inf.brain {
                args.brain = Some(
                    crate::project_gf::resolve_against(&base, b)
                        .to_string_lossy()
                        .into_owned(),
                );
            }
...
fn run_gf_init(output: Option<PathBuf>, name: Option<String>) -> Result<(), String> {
    ...
    let body = crate::project_gf::init_template(&default_name);
...
///
/// # Example (from SpaceKit)
/// ```ignore
/// growformer::cli_impl::run_from_argv(["growformer", "--train-brain", "--project", "scripts/foo.gf.toml"])?;
/// ```
pub fn run_from_argv<I, T>(argv: I) -> Result<(), String>
where
    ...
    T: Into<OsString> + Clone,
{
    ...
    if let Some(cmd) = &cli.command {
        match cmd {
            Commands::Init { output, name } => {
                run_gf_init(output.clone(), name.clone())?;
                return Ok(());
            }
        }
    }
    ...
    let gf_overlay = match args.project.clone() {
        Some(ref p) => Some(apply_gf_project(&mut args, p)?),
        None => None,
    };

    crate::inference::set_inference_toml_cli_paths(
        args.inference_toml.clone(),
        args.inference_defaults_toml.clone(),
    );
    crate::inference::set_inference_guardrails_jsonl_path(args.inference_guardrails_jsonl.clone());
...
fn apply_optional_brain_plugins_toml(
    ...
    let m = crate::inference::parse_plugins_manifest_bytes(s.as_bytes())?;
        ...
        if let Some(r) = s.expected_response.as_deref() {
            match runtime.encode_and_bridge(&s.text) {
                ...
                Err(_) => {
                    ...
                    gen_pairs.push((
                        vec![0.0; crate::dimension::group_gen::GEN_COND_DIM],
                        lattice_text,
...
fn execute_tool(call: &crate::dimension::tool::ToolCallInfo) -> crate::dimension::tool::ToolResult {
    crate::tools_builtin::execute_tool(call)
}

fn run_conversation_repl(rt: &mut crate::runtime::Runtime) {
    let svc = &mut rt.svc;
...
fn handle_repl_command(rt: &mut crate::runtime::Runtime, cmd: &str) {
    use crate::service::OceanProfile;
            ...
            if svc.conversation.is_empty() {
                ...
                for (i, turn) in svc.conversation.history.iter().enumerate() {
                    let role = match turn.role {
                        crate::service::TurnRole::User => "user",
                        crate::service::TurnRole::Agent => "agent",
                    };
...
fn discover_group_names(samples: &[crate::dimension::LanguageSample]) -> Vec<String> {
...
fn train_brain(
    ...
    crate::inference::print_train_inference_disk_summary();
    ...
    let action_samples: Vec<(Vec<f32>, crate::dimension::action::ActionType)> = bridged_embeddings
        .iter()
        .zip(samples.iter())
        .map(|(emb, s)| {
            let at = action_target_to_type(s.action_target.as_deref().unwrap_or("coding"));
    ...
    use crate::dimension::language::{GroupAdapter, DEFAULT_ADAPTER_RANK};
            ...
            let action_data: Vec<(&[f32], usize)> = action_samples
                ...
                .map(|(emb, at)| {
                    let idx = match at {
                        ...
                        crate::dimension::action::ActionType::CodingAssist => 1,
                        crate::dimension::action::ActionType::GeneralAssist => 2,
                        crate::dimension::action::ActionType::ToolCall => 3,
                        crate::dimension::action::ActionType::Fallback => 4,
                    };
                    (emb.as_slice(), idx)
                })
                .collect();
