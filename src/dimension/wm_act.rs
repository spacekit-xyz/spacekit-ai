...
fn vg_plan(a_in: &ActionEnergyAdapter, a_out: &ActionEnergyAdapter, z: &[f32]) -> WmAction {
    ...
    for a in 0..ACTION_DIM {
        ...
        let e = 0.5
            * (a_in.planning_energy(z, act) + a_out.planning_energy(z, act));
        if e < best_e - 1e-6 {
            best_e = e;
            best = act;
        }
...
fn goal_disk(obs: &[f32], next: &[f32]) -> f32 {
...
pub fn run_phase3t_disk_act_seed(seed: u64, work_dir: &Path) -> ActSeedResult {
    ...
    let pin_stable = loaded.verify().is_ok()
        && loaded.encoder_fingerprint == bundle.encoder_fingerprint;
...
pub fn run_phase3t_visuomotor_act_seed(seed: u64, work_dir: &Path) -> ActSeedResult {
    ...
    // Gripper starts on the object so contact is immediate; oracle = push toward origin.
    let oracle_push = |ox: f32, oy: f32| -> u8 {
        if ox.abs() >= oy.abs() {
            if ox > 0.0 {
                0
            } else {
    ...
    while ranked.len() < 240 {
        ...
        let gx = ox + rng.gen_range(-0.05..0.05);
        let gy = oy + rng.gen_range(-0.05..0.05);
        let frame = render_visuomotor(gx, gy, ox, oy);
        let z = enc.encode(&frame);
        let best_a = oracle_push(ox, oy);
        let mut nexts = Vec::with_capacity(ACTION_DIM);
        for a in 0..ACTION_DIM {
            let act = WmAction::from_u8(a as u8);
            let (gx2, gy2, ox2, oy2) = step_visuomotor(gx, gy, ox, oy, a as u8);
            nexts.push((act, enc.encode(&render_visuomotor(gx2, gy2, ox2, oy2))));
        }
    ...
    ad.train_true_next_ranked(&ranked, &[], 40, 0.06);
    ad.train_rank_only(&ranked, 160, 0.1);
    ...
    let pin_stable = loaded.verify().is_ok()
        && loaded.encoder_fingerprint == pin
        && fp_vision(&enc) == pin;
...
impl ActingHostSession {
    ...
    pub fn handle(&mut self, req: ActingHostRequest) -> ActingHostResponse {
        match req {
            ActingHostRequest::LoadActing { path } => match ActingWmBundle::load(Path::new(&path)) {
                Ok(b) => {
