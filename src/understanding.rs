...
impl SmallMlp {
    pub fn new(input_dim: usize, hidden_dim: usize, output_dim: usize, _seed: u64) -> Self {
        ...
        Self { lattice, labels, input_dim, hidden_dim, output_dim }
    }
    ...
    pub fn build(input_dim: usize, output_dim: usize, samples: &[(&[f32], usize)]) -> Self {
        ...
        Self { lattice, labels, input_dim, hidden_dim: 0, output_dim }
    }
    ...
    fn predict_shared(&self, input: &[f32]) -> usize {
        if self.lattice.programs.is_empty() { return 0; }
        let mut best_idx = 0;
    ...
    fn predict_with_confidence_shared(&self, input: &[f32]) -> (usize, f32) {
        if self.lattice.programs.is_empty() { return (0, 0.0); }
        let mut best_idx = 0;
        let mut best_sim = f32::NEG_INFINITY;
        for (i, prog) in self.lattice.programs.iter().enumerate() {
            let sim = cosine_sim(input, &prog.ema_centroid);
...
impl UnderstandingLayer {
    ...
    pub fn build(samples: &[(&[f32], &str)], raw_dim: usize) -> Self {
        ...
        let topic_samples: Vec<(&[f32], usize)> = samples.iter()
            .filter_map(|&(raw, intent)| topic_map.get(intent).map(|&idx| (raw, idx)))
            .collect();
        let topic_clf = SmallMlp::build(raw_dim, num_topics, &topic_samples);
        println!("    [understanding/topic] Paramecium one-pass: {} programs from {} samples",
            topic_clf.lattice.program_count(), topic_samples.len());

        // Build verb classifier — one-pass Paramecium develop
        let verb_samples: Vec<(&[f32], usize)> = samples.iter()
            .map(|&(raw, intent)| {
        ...
        println!("    [understanding/verb] Paramecium one-pass: {} programs from {} samples",
            verb_clf.lattice.program_count(), verb_samples.len());
